...
The layout has exactly one finite, non-negative integer geometry record for every component and one edge record for every relationship. It rejects invented, missing, duplicate, malformed, or cyclic IDs/parents/waypoints. One sanitized repair is allowed. Only a validated layout is rendered, and output writes atomically.
